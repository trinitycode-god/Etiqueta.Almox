...
// ---------------------------------------------------------
// AJUSTE AUTOMÁTICO DE FONTE (encolhe se o texto não couber)
// ---------------------------------------------------------
const fontesOriginais = {};

function capturarFontesOriginais() {
  [el.etqCodigo, el.etqDescricao].forEach((elemento) => {
    fontesOriginais[elemento.id] = parseFloat(
      getComputedStyle(elemento).fontSize
    );
  });
}

function ajustarFonteParaCaber(elemento, tamanhoMinimoPx = 11) {
  const tamanhoMaximo = fontesOriginais[elemento.id];
  if (!tamanhoMaximo) return;

  let tamanho = tamanhoMaximo;
  elemento.style.fontSize = tamanho + "px";

  while (
    tamanho > tamanhoMinimoPx &&
...
function ajustarTodasAsFontes() {
  ajustarFonteParaCaber(el.etqCodigo, 16);
  ajustarFonteParaCaber(el.etqDescricao, 11);
}

let resizeTimer = null;
window.addEventListener("resize", () => {
  clearTimeout(resizeTimer);
  resizeTimer = setTimeout(ajustarTodasAsFontes, 120);
});
